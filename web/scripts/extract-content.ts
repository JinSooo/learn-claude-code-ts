...
function extractClasses(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) {
      const name = m[1];
      const startLine = i + 1;
      // Find end of class by tracking brace depth
...
// Looks for "name": "tool_name" or name: "tool_name" patterns in object literals
...
function detectLocale(relPath: string): "en" {
  return "en";
}
